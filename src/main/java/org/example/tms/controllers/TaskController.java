...
//        return ResponseEntity.ok(taskService.getTop5PriorityTasks());
//    }
//
...
import org.example.tms.dto.response.TaskResponse;
import org.example.tms.entity.TaskStatus;
import org.example.tms.security.CustomUserDetails; // Добавили импорт
import org.example.tms.service.TaskService;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.security.access.prepost.PreAuthorize;
import org.springframework.security.core.annotation.AuthenticationPrincipal; // Добавили импорт
import org.springframework.web.bind.annotation.*;
...
public class TaskController {
    ...
    @PreAuthorize("@securityService.canUpdateTaskStatus(#id)")
    @PatchMapping("/{id}/status")
    public ResponseEntity<TaskResponse> updateStatus(@PathVariable Long id, @RequestParam TaskStatus status) {
        return ResponseEntity.ok(taskService.updateStatus(id, status));
    }
